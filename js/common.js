...
if(!getcookie('shop_id')&&getcookie('mid')&&getcookie('token')){

   $.ajax({
        url: WapSiteUrl+"/api/index.php?act=shoper_manage&op=get_shop_info",
        type: 'get',
        data:{flag:"wap",token:getcookie('token'),mid:getcookie('mid')},
        async:false,
        dataType: 'json',
...
// try {
//   if(native_flag=='0'&&FHMall){
//     //  FHMall.enableShareButton(null);
//   }
// }
...
if(firstShow==""&&ss.indexOf("fh")==-1){
    var dom = '<div class="downLoad-fix layout">'
        +'<img src="../../images/wap/downLoad1.jpg" style="width:100%" id="downLoad-btn"/>'
        +'<img src="../../images/wap/top-x.png" class="downLoad-x" id="closeDownLoad" />'
        +'</div>';
    $("body").prepend(dom);
    $("#closeDownLoad").click(function(){
    ...
    $('#downLoad-btn').click(function(){
        location.href='http://www.fenhongshop.com/wap/tmpl/index/gzherweima.html';
    })
}
...
var statisticsDom='<div style="display:none;">'
    +'<script src="https://s4.cnzz.com/z_stat.php?id=1261162956&web_id=1261162956" language="JavaScript">'
    +'</script>'
    +'</div>';
            ...
            if(url&&url.split("!")[1]){
                try{
                    ...
                    if(suffix =="nation"){
                        ...
                        location.href=url+"&timelink="+Math.random();
                    }
                    e.preventDefault();
                }catch(e){
...
//模版时间
function talentTime(date){
    date = new Date(date*1000);
    var map = {
        "M": date.getMonth() + 1, //月份
    };

    format = '<span class="f30">MM</span>月·yyyy'.replace(/([yM])+/g, function(all, t){
        var v = map[t];
        if (v !== undefined) {
            if (all.length > 1) {
                v = '0' + v;
...
var deductid = GetQueryString("deductid");
if(deductid){
    localStorage.setItem("deductid",deductid);
}
...
(function() {
    ...
    function logLogin(signage){
        if(!isWeiXin()||signage){
			...
			scriptFile.setAttribute("src",'https://www.fenhongshop.com/wap/js/apiLink.js');
			document.getElementsByTagName("head")[0].appendChild(scriptFile);
			var scriptFile = document.createElement('script');
			scriptFile.setAttribute("type","text/javascript");
			scriptFile.setAttribute("src",'https://www.fenhongshop.com/wap/src/js/tmpl/login/checklogin.js');
			document.getElementsByTagName("head")[0].appendChild(scriptFile);
//          $.getScript("http://www.fenhongshop.com/wap/js/apiLink.js");
//          $.getScript("http://www.fenhongshop.com/wap/src/js/tmpl/login/checklogin.js");
        }else{
    ...
    window['FL']['addShadeLog'] = addShadeLog;//分享提示
